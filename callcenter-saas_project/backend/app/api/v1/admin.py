...
async def update_user_status(
    ...
    try:
        ...
        # Convert current_user.id to string for comparison
...
async def get_admin_stats(
    ...
    try:
        ...
        demo_bookings_this_week = await demo_bookings_collection.count_documents({
            "created_at": {"$gte": week_ago}
        })
        demo_bookings_this_month = await demo_bookings_collection.count_documents({
            "created_at": {"$gte": month_ago}
        })
        ...
        # Generate some mock stats for features not yet implemented
        mock_stats = generate_mock_stats()
        ...
        stats_data = {
            ...
            # Real demo booking stats
            "total_demo_bookings": total_demo_bookings,
            "demo_bookings_this_week": demo_bookings_this_week,
            "demo_bookings_this_month": demo_bookings_this_month,
            
            # Growth rate calculations
            "user_growth_rate": round(new_users_this_week / max(total_users - new_users_this_week, 1) * 100, 1) if total_users > 0 else 0,
            "conversion_rate": round((professional_users + enterprise_users) / max(total_users, 1) * 100, 1) if total_users > 0 else 0,
            
            # Mock stats for features not yet implemented
            **mock_stats,
